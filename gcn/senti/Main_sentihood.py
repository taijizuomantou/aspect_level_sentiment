...
fout_train = open("input_data/head_train.txt","r")
fout_test = open("input_data/head_test.txt","r")
def convert_examples_to_features_sentihood(examples, label_list, max_seq_length, tokenizer,typee):
   ...
   # my_dict = {}
    fout = fout_test
    if(typee == "train"): fout = fout_train
...
#--seed 42
    ...
    all_pos = torch.tensor([f.aspect for f in train_features], dtype=torch.long)
    all_dis = torch.tensor([f.dis for f in train_features], dtype=torch.long)
    all_pos_sent = torch.tensor([f.my_pos_sent for f in train_features], dtype=torch.float)
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all__detection_label_ids,all_aspects,all_pos,all_dis,all_pos_sent,all_noun_label,head)
   ...
   # model = torch.load("model_data/attention_add_model6")
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
           ...
           # print()
            input_ids, input_mask, segment_ids, label_ids, exist_ids, all_aspects,pos,dis,all_pos_sent,all_noun_label,head = batch
            loss, _ ,_= model(input_ids, segment_ids, input_mask, label_ids, exist_ids, all_aspects,all_noun_label,all_pos_sent,head)
            if n_gpu > 1:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                global_step += 1
           # break
        torch.save(model,"model_data/latestsentimodel"+str(epoch))
         #eval_test
        if args.eval_test :
            model.eval()
