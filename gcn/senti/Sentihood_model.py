...
class BertForSequenceClassification(nn.Module):
    ...
    def __init__(self, num_labels):
        ...
        self.embedding2 = nn.Embedding(2,768)
    ...
    #    print(aspects)
       ...
       # pooled_output = self.dropout(pooled_output)
        ...
        #encode2 = self.dropout(encode)
        detection_logits = self.classifier_detection(pooled_output)
        sentiment_logits = self.classifier_sentiment(pooled_output)
      #  print(aspects.size())
        size = aspects.size(0)
        zeros_x = torch.zeros(size).cuda()
        ones_y = torch.ones(size).cuda()
        loc_one_or_two = torch.where(aspects > 3, ones_y,zeros_x).long()
        loc_embed = self.embedding2(loc_one_or_two)
        four_aspects = torch.where(aspects > 3,aspects - 4, aspects).long()
        aspect_embed = self.embedding(aspects)+loc_embed
      ...
      #  print(encode.size())
        ...
        Mc = self.wh_c(encode)*(full_aspect_embed)+self.we_c(Md) + X
        attention_c = self.softmax_c((self.w_c(Mc))) #+ attention_d
      #  attention_c = attention_c + attention_d
        temp_encode = encode.permute(0,2,1)
        r_c = torch.bmm(temp_encode,attention_c).squeeze(-1)#+r_d
      ...
      #  print(attention_d.size())
        ...
        loss = loss + loss_fct_2(sentiment_logits,class_labels)*1.25#*0.25
        #print(attention_d.size())
