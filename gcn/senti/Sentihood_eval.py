...
def get_not_true(y_true, y_pred):
    my_y_true = []
    for item in y_true:
        if item == 2:
            my_y_true.append(0)
    ...
    for i in range(0,len(y_true)):
        if(my_y_true[i] != y_pred[i]):
            idx.append(i)
 #   get_idx_all(idx,y_pred)
...
def sentihood_macro_F1_3(y_true, y_pred):
    """
...
#pred_data_dir = "senti_results/4_3e-5_loss*0.5/NLI_M/test_ep_4.txt"
#pred_data_dir = "senti_results/11_3e-5_1_no_sent/NLI_M/test_ep_4.txt"
...
get_not_true(y_true,detect_pred)
