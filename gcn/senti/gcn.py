...
class GCNAbsaModel(nn.Module):
    ...
    def forward(self, head, gcn_input,typee,loc1,loc2):
   #     print("head"+ str(head.size()))
      #  print(head)
        def inputs_to_tree_reps(head,typee,loc1,loc2):
       ...
       # adj = [inputs_to_tree_reps(onehead,typee,loc).reshape(1, MAX_LENGTH, MAX_LENGTH) for onehead in head]
        ...
        for i,onehead in enumerate(head):
            oneloc1= loc1[i]
            oneloc2 = loc2[i]
            adj.append(inputs_to_tree_reps(onehead,typee,oneloc1,oneloc2).reshape(1, MAX_LENGTH, MAX_LENGTH) )
...
class GCN(nn.Module):
    def __init__(self, num_layers = 2,gcn_dropout = 0.1):
