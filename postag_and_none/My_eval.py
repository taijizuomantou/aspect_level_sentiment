...
def semeval_PRF(y_true, y_pred,score,detect_score):
    ...
    fout = open("z_p_wrong.txt","w")
    fout_idx = open("z_p_idx.txt","w")
    df = pd.read_csv(fin,sep='\t',header=None).values
    for i in range(len(df)):
        if i in idx:
            fout_idx.write(str(i))
            fout_idx.write("\n")
            fout.write(df[i][2])
            fout.write("\t\t")
            fout.write(df[i][3])
...
pred_data_dir = "results/50_base/NLI_M/test_ep_5.txt"
pred_data_dir = "results/semeval2014/NLI_M20/test_ep_6.txt"  
#pred_data_dir = "ceshi/54_base_53_delete_un_noun_label/NLI_M/test_ep_4.txt"
