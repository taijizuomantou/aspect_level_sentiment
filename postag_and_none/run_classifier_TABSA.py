...
def main():
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all__detection_label_ids,all_aspects,all_noun_label,all_sent_label)
   ...
   # model = torch.load("model_data/new_sem/attention_add_model6")
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            input_ids, input_mask, segment_ids, label_ids, exist_ids, all_aspects,all_noun_label,all_sent_label = batch
            loss, _ ,_= model(input_ids, segment_ids, input_mask, label_ids, exist_ids, all_aspects,all_noun_label,all_sent_label)
            if n_gpu > 1:
        ...
        if args.eval_test :
            ...
            with open(os.path.join(args.output_dir, "test_ep_"+str(epoch)+".txt"),"w") as f_test:
                for input_ids, input_mask, segment_ids, label_ids, exist_ids,all_aspects,all_noun_label,all_sent_label in test_dataloader:
                    ...
                    with torch.no_grad():
                        tmp_test_loss, detect_logits,sent_logits = model(input_ids, segment_ids, input_mask, label_ids, exist_ids,all_aspects,all_noun_label,all_sent_label)
